...
from .eval_kd import metrics_for_eval_kd
...
def _train_epoch_kd (models, optimizer, feat_fn, loss_fn, data_loader, c_epoch, t_epoch, device, topk=2):
    """
    ...
    :param feat_fn: feature extraction functions, for student and teacher models.
    ...
    feat_fn_s = feat_fn['feat_fn_s']
    feat_fn_t = feat_fn['feat_fn_t']
    ...
    with tqdm(total=len(data_loader), ascii=True, desc='Epoch {}/{}: '.format(c_epoch, t_epoch), ncols=100) as t:
        ...
        for batch, data in enumerate(data_loader, 0):
            ...
            if len(metadata_batch):
                ...
                # Doing the forward pass
                # print(model)
                out_s = feat_fn_s(model, imgs_batch, student=True)
                out_t = feat_fn_t(model_t, imgs_batch, student=False)

            # pdb.set_trace()
            # Computing loss function
            loss = loss_fn(out_s, out_t, labels_batch) # KD

            # Computing the accuracy
            acc, topk_acc = accuracy(out_s['logits'], labels_batch, topk=(1, topk))
...
def fit_model_kd (models, train_data_loader, val_data_loader, optimizer=None, feat_fn=None, loss_fn=None, epochs=10,
    ...
    :param t_epoch (int): the total number of epochs
    :param device (torch.device): the device to carry out the training
    :param models (torch.nn.Module): teacher, student models, but only the student model requires training.
    ...
    :param feat_fn: feature extraction function
    ...
    model = models['model_s']  # only student model needs training
    ...
    if feat_fn is None:
        logger.info('Feature module was set as None.')
        raise Exception("feat_fn is not set. Check it, please!")
    ...
    while epoch < epochs:
        ...
        train_metrics = _train_epoch_kd(models, optimizer, feat_fn, loss_fn, train_data_loader, epoch, epochs, device, topk)

        # After each epoch, we evaluate the model for the training and validation data
        val_metrics = metrics_for_eval_kd (models, val_data_loader, device, feat_fn, loss_fn, topk,
