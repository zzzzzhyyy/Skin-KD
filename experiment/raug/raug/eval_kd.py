...
def metrics_for_eval_kd (models, data_loader, device, feat_fn, loss_fn, topk=2, get_balanced_acc=False, get_auc=False):
    """
        ...
        with torch.no_grad():
            ...
            for data in data_loader:
                ...
                if len(meta_data_batch):
                    ...
                    out_s = feat_fn_s(model, images_batch, student=False)
                    out_t = feat_fn_t(model_t, images_batch, student=False)
                    pred_batch = out_s['logits']


                # Computing the loss
                # L = loss_fn(pred_batch, labels_batch)
                L = loss_fn(out_s, out_t, labels_batch)
...
# Testing the model
def test_model_kd (models, data_loader, checkpoint_path=None, feat_fn=None, loss_fn=None, device=None, save_pred=False,
