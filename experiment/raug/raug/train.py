...
from . import LogitRegularization
    ...
    with tqdm(total=len(data_loader), ascii=True, desc='Epoch {}/{}: '.format(c_epoch, t_epoch), ncols=100) as t:
        ...
        for batch, data in enumerate(data_loader, 0):
            ...
            if len(metadata_batch):
                ...
                logits0, logits1 = model(imgs_batch, metadata_batch)
            else:
                ...
                # Doing the forward pass
                logits0, logits1 = model(imgs_batch)

            # Computing loss function
            loss0 = loss_fn(logits0, labels_batch)
            loss1 = LogitRegularization(logits1, labels_batch)
            alf = 0.05
            loss = loss0 * alf + (1 - alf) * loss1

            # Computing the accuracy
            acc, topk_acc = accuracy(logits0, labels_batch, topk=(1, topk))
