...
class MyResnet (nn.Module):
    ...
    def __init__(self, resnet, num_class, neurons_reducer_block=256, freeze_conv=False, p_dropout=0.5,
        ...
        if neurons_reducer_block > 0:
            ...
            self.classifier1 = nn.Linear(neurons_reducer_block + _n_meta_data, num_class)
        else:
            self.classifier = nn.Linear(n_feat_conv + _n_meta_data, num_class)

            self.classifier1 = nn.Linear(n_feat_conv + _n_meta_data, num_class)
...
def extract_feats_resnet50(model, data, student=True):
    ...
    for name, module in modules:
        if name == 'features':
            ...
            model.classifier.register_forward_hook(get_activation('logits0'))

        elif name == 'classifier1':
            model.classifier1.register_forward_hook(get_activation('logits1'))
